import random
import math           #Uçağın gideceği hava alanı noktası bulma
n = 10
d = {}
u = {}
...
for i in range(1, n+1):
  for j in range(1, n+1):
    ...
    print("%3d%3d [%3d,%3d] %3d %3d [%3d,%3d] uzaklik :%10,4f durum:%1d")
print("siraliyor...")
print("--------------------")
...
for i in range(1,k):
  for j in range(i+1,k+1):
    if u[i][8] > u[j][8]:
      ...
      g = u[i][0]; u[i][6] = u[j][6]; u[j][6] = g
      g = u[i][0]; u[i][7] = u[j][7]; u[j][7] = g
      g = u[i][0]; u[i][8] = u[j][8]; u[j][8] = g
      g = u[i][0]; u[i][9] = u[j][9]; u[j][9] = g
m = 0
for i in range(1,k+1):
  if u[i][9] == 1:
    m += 1
    print("SN :%3d Adet :%3d %3d %3d [%3d,%3d] %3d %3d [%3d,%3d] uzaklik :%10")